...
spawn 返回代表孩子已启动，不代表完成。先保存关系，再创建子会话与设置，最后启动 Runner；恢复校验循环与超深，缺任务名报错，不推导旧记录。孩子只接收委派说明，不复制父历史。

回报用 TaskID 与轮次派生稳定 ID，直属父账本落账才算确认；父闲时保留，下一次主动启动时在首次模型调用前接收。wait 等新完成通知，不持有模型请求，Steer 可唤醒，超时不停止孩子。
