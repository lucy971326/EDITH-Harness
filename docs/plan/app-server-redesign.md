...
更新于 2026-09-10。方向已确定；完成事实只看 `STATUS.md`，剩余施工按实施计划继续。
...
- JSON-RPC 封套与请求配对交给选定协议库；app-server 不在库外维护第二套兼容层。
- 每个 Client 只有一个基础设施 `Connection`，只管初始化、RPC 连接、订阅、发送保护、待回答请求配对和断线清理。
- `Connection` 绝不保存 Session、Run、设置或产品状态，也不决定 Start / Steer / Stop。业务规则只属于 Product 或对应公共服务。
- 业务服务决定为什么提问、回答是否合法以及之后如何继续。
- JSON-RPC `id` 只配对一次响应；有副作用操作另用业务操作 ID 防重。
- 首版本机单用户、多 Client，只监听回环地址；不做跨设备、账号和逐接口权限系统。
- 本机使用数字回环地址、浏览器 Origin 校验和初始化超时；后台发现与唯一实例在正式 Client 建立时再定。
