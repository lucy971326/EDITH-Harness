...
>
> 这套设计的重点是让事实各有唯一归属：`task.json` 只保存父子关系；子账本保存孩子的聊天和结果；父账本中的 MessageID 证明回报已经送达；活跃 Run 只属于 Runner。我们没有另存一份任务状态或结果，避免双写，也让重启后的恢复更明确。当前第一版固定为一层委派，先避免任务树、递归停止和多层面板的复杂度。
...
每派出一个孩子，Subagents 会保存一份关系文件：`<taskID>.json`。
