...
- React 保留一份类型化 RPC Client、手写契约和状态投影；从 `RPCClient` 抽出最薄的消息传输接口，分别接 WebSocket 与 Wails。headless 有自己的 Go Client，不复制业务方法。
- Desktop 选择 Wails v3：React 在 WebView 中使用 `Stream("rpc")` 收发消息；`clients/desktop` 注册 Go Stream 处理器并适配 `ObjectStream`，不建业务方法绑定。`appserver` 不依赖 Wails。Stream 打开后发送 `initialize`。
- Wails 加载共用 React 静态资源；Stream 借助其资源服务器通信，不为桌面业务开放回环端口。普通 Web 仍用 WebSocket。固定经验证的 v3 Beta 版本，不自动跟随每日预发行版。
...
4. 接入 Wails v3：共用 React 页面，加入 Stream 传输适配、桌面 Go 接入、窗口启动与关闭。先验证 JSON-RPC 往返、通知顺序、断线、背压和关闭，再检查 Monaco Worker、图片压缩、剪贴板、外部链接与目录选择在实际 WebView 中的表现；确有差异才加平台小适配。需要控制已运行后台时再设计本机传输，不提前实现远程服务或 TUI。

## 目录方向

```text
cmd/harness/                 共享显式装配；增加运行入口，不复制服务清单
...
- 多个 Client 连接同一后台时，请求结果只回发起者，订阅事件各自收到；重连以 Snapshot 补齐，不靠传输重放写请求。
