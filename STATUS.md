...
## 架构精简：存储与编辑器收口（2026-09-24）

- 上一批主干重构已提交为 `616b166`；本批改动尚未提交。
- `persist` 只保留可靠文件操作。账本格式与元数据归 Session，会话设置归 settings，Agent 文件归 agents；删除 `agents/config` 别名包和重复元数据形状。Runner 直接读写运行记录与 Diff，移除原始字节转发依赖。Agent 引用／目录与删除继续互斥，运行记录与快照继续共用事务锁。
- 编辑器文件生命周期集中到 `editor/use-files.ts`，标签页只保留编排与关闭确认。合并重复监听索引，保存按文件对象跟踪；修复冲突重读等待自身、关闭重开后迟到响应更新新文件的问题，保留工作区草稿与哈希冲突检查。UI 样式未改。
