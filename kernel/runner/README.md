...
Runner 是唯一写账者；Loop 不碰 Session，UI 不听账本。每次 Run 会生成一个 `RunID`：用户消息落账后才发布 `RunStarted(AfterEntrySeq)`；每个事件附带 `StepSeq / BlockSeq`，工具结果按 `ToolCallID` 回到原块。Steer 真正落账才成为同一 Run 的下一个画面边界。对外 `RunEvent` 是稳定契约，不泄漏 Loop 内部事件；关闭 Runner 会取消并等待它管理的全部 Run。
