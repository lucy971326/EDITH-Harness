...
- 选中会话 ID 存本标签页 `sessionStorage`，刷新后恢复订阅；首次不自动选中，只在明确不存在时清除。输入草稿和图片预览按会话保存在内存，刷新不承诺恢复。
...
- 输入确认后才清掉本次提交的文字；提交期间的新编辑和其他会话草稿不能被迟到确认清除。等待确认期间禁止重复提交；超时／断线保留文字并提示核对历史，不按正文猜测是否接受。
- 忙时携带 `expectedRunID` 直接插话，无等待队列；原轮已结束／关闭插话／换轮返回冲突，文字留下，由用户确认后再次发送。停止响应只是已接收请求，界面等 Snapshot／结束事件，不把抢先成功改成取消。
- 模型与思考合用一个菜单，目录来自 `model/list`。没有有效设置就明确选择，不填默认值；运行中禁改，Agent 沿用会话设置。标题和已保存设置在耐久用户消息或运行结束后刷新，不在每条 Delta 上拉列表。
- Enter 提交、Shift＋Enter 换行，输入法选词不提交；向上阅读时暂停自动滚动。图片发送接入前，有附件就阻止整条提交并说明，不能偷偷只发文字。
