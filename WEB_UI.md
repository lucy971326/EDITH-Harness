...
左侧项目栏、右侧辅助区及其文件树复用同一分隔线交互，支持鼠标拖动与键盘方向键。拖动时只移动分隔指示线，松手后一次提交宽度，避免长内容在每个像素上重复布局。右侧默认取初始窗口宽度的四分之一，限制在 320–480px；之后可拖动并保存到新版阅读布局宽度偏好。桌面分栏始终为聊天区保留至少 300px，辅助区的实际最大宽度随窗口和左栏宽度计算；窗口不超过 950px，或剩余空间不足以同时容纳 300px 聊天区和 320px 辅助区时，辅助区改为覆盖模式。辅助区第一行是所有视图共用的混合标签栏；编辑器在自己的第二行工具栏管理路径、状态和文件树折叠。代码区在文件树左侧；Diff 变更列表默认收起，由自己的工具栏展开。终端等其他视图不显示编辑器工具栏或文件树。Monaco 使用与 Harness Token 对应的亮／暗主题，并延迟加载本地资源。
