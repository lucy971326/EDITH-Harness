...
设置页另有 Hooks：分别编辑全局和项目 `PreToolUse` 命令、参数、工具名、启用与顺序。项目来源可选择工作区；外部配置变化后需要在页面重新确认。保存冲突要求重新加载。Hook 故障在当前 Run 短暂提示，设置页显示最近原因；两者都不写入聊天账本。页面明确说明项目脚本在宿主运行，脚本内容变化不会自动触发重新信任。
