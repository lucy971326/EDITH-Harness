...
- 引用显示在输入框上方，只能整块删除；代码选区可展开纯文本预览。同类路径去重；代码选区还要求范围和内容完全相同才去重。同名文件显示完整相对路径，外部已打开文件显示绝对路径。允许只发引用，文件／目录不自动读取内容。
- 引用按设计书的 v1 文本段通过原有 `send.text` 发送。用户消息和 Steer 使用同一解码组件，完整合法段显示为只读标签和可展开代码，未知格式显示原文；助手／工具正文不解析。刷新和分叉不另存引用状态。
