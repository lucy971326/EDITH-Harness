...
左侧项目栏、右侧辅助区及其文件树复用同一分隔线交互，支持鼠标拖动与键盘方向键。桌面分栏始终为聊天区保留至少 300px，辅助区的实际最大宽度随窗口和左栏宽度计算；剩余空间不足以同时容纳 300px 聊天区和 520px 辅助区时，辅助区自动改为覆盖模式。辅助区第一行是所有视图共用的混合标签栏；编辑器在自己的第二行工具栏管理路径、状态和文件树折叠。代码区在文件树左侧；终端等其他视图不显示编辑器工具栏或文件树。Monaco 使用与 Harness Token 对应的亮／暗主题，并延迟加载本地资源。
