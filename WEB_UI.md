...
```text
白色内容区 + 暖灰侧栏 + 近黑文字
细线区分工作区，中性灰用于选中，蓝色用于链接、焦点与活动标签
界面固定 Ginto → Noto Sans SC
代码使用打包的 JetBrains Mono → Noto Sans SC；普通界面以细线为主，浮层保留阴影
```

Token 是颜色、排版、间距、圆角、边框、阴影、动画和主题的唯一视觉事实来源。亮色、暗色、跟随系统只切换 Token；产品不得各写一份 dark 样式。

字体随前端打包，不提供字体切换；中文正文、控件、编辑器注释和 Diff 统一使用 Noto Sans SC。Monaco 读取同一代码字体 Token，字体分片加载后重新测量字宽。正文 14px、行高 1.55，导航 12px；正文和输入框共用最大 660px 的居中阅读列及响应式水平留白。导航与聊天间距保持紧凑，顶栏共用高度，表格以横线分隔，Diff 摘要保留轻边框和独立操作入口。
...
设置页另有 Hooks：全局与项目 Hook 按顺序列出，默认收起，展开单项编辑 `PreToolUse` 命令、参数、工具名、启用与顺序。项目来源可选择工作区；外部配置变化后需要在页面重新确认。保存冲突要求重新加载。Hook 故障在当前 Run 短暂提示，设置页显示最近原因；两者都不写入聊天账本。项目脚本在宿主运行，脚本内容变化不会自动触发重新信任。
...
`workspace-tabs.tsx` 负责标签、面板和关闭确认；`editor/use-files.ts` 负责工作区草稿、打开、自动保存、冲突与文件监听。草稿按工作区保存，异步结果只更新原来的文件对象；关闭再打开同一路径不会接收旧结果。

保存使用内容哈希检测冲突。监听重读等待正在进行的保存；保存被拒绝则直接读取磁盘显示冲突，不等待自身。未挂载时停止定时保存与监听，已发出的写请求只完成原有操作。
